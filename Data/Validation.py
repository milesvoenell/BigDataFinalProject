...
# ----------------------------
# Helper to convert finish time to seconds
# ----------------------------
...
def validate_row_dict(row: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    # Clean empty strings and -0 in State
...
def validate_csv(input_file: str, output_file: str) -> pl.DataFrame:
    ...
    # Rename Finish Time column for consistency
    ...
    # Replace any remaining None values with string "None"
    df_validated = df_validated.fill_null("None")

    logger.info(f"Saving validated CSV to: {output_file}")
